...
-- Add new check constraint with more precise validation and proper decimal handling
ALTER TABLE rollup_walls
...
CHECK (
  CASE 
    ...
    WHEN wall_location = 'Endwall' AND type = 'Quonset' THEN
      frame_height::numeric = ANY (ARRAY[0.8, 3.5, 4.5, 5.0, 6.0]::numeric[])
      AND wall_height > 0 
      AND wall_height <= frame_height
    ELSE false
  END
...
-- Ensure both columns are numeric with proper precision and scale
ALTER TABLE rollup_walls

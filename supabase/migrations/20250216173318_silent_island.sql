...
ALTER COLUMN wall_length TYPE integer USING COALESCE(wall_length, 1)::integer;
...
-- Add simplified wall height validation that works for both sidewalls and endwalls
ALTER TABLE rollup_walls
...
CHECK (
  CASE 
    ...
    WHEN wall_location = 'Sidewall' AND type = 'Quonset' THEN 
      wall_height = ANY (ARRAY[8.0, 3.5, 4.5, 5.0, 6.0]::numeric[])
    -- For endwalls, just ensure height is positive and doesn't exceed frame height
...
-- Add frame height validation for endwalls
ALTER TABLE rollup_walls
  ...
  (wall_location = 'Endwall' AND frame_height IS NOT NULL AND
   CASE
     ...
     WHEN type = 'Quonset' THEN 
       frame_height = ANY (ARRAY[8.0, 3.5, 4.5, 5.0, 6.0]::numeric[])
     ELSE false
   END)
);
...
IS 'Height of the roll-up wall in feet. For sidewalls: specific heights based on type. For endwalls: must not exceed frame height.';

COMMENT ON COLUMN rollup_walls.frame_height
IS 'Frame height in feet. Required for endwalls. Values depend on type (Guttered: 8, 10, 12, 14 ft; Quonset: 8, 3.5, 4.5, 5, 6 ft).';
